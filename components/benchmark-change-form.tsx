      ...
      <section className="form-section"><div className="section-number">02</div><div className="section-content">
        ...
        <div className="portfolio-list">{client?.portfolios.map((portfolio) => {
          ...
          return <article className={`portfolio-card ${selected ? "is-selected" : ""}`} key={portfolio.id}>
            ...
            <div className="benchmark-row"><div className="benchmark ist"><span>IST</span><b>{portfolio.currentBenchmark.code}</b><small>{portfolio.currentBenchmark.name}</small></div><span className="arrow">→</span><label className="benchmark soll"><span>SOLL</span><select disabled={!selected} value={target ?? ""} onChange={(event) => setTarget(portfolio.id, event.target.value)}><option value="">Kies benchmark</option>{benchmarks.map((benchmark) => <option key={benchmark.id} value={benchmark.id}>{benchmark.code} — {benchmark.name}</option>)}</select></label></div>
          </article>;
        })}</div>
      </div></section>
