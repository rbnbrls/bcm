...
- **Read**: `getClientOnboardingStagingByStagingId()` (by primary key, returns `null` when absent), `getClientOnboardingStagingByClientCode()` (all rows for a client, optionally filtered by status), and `getClientOnboardingStagingByChangeRequestId()` (the single row for a change request — used by the apply step).
...
- All helpers use parameterized queries (postgres.js tagged templates) — no string interpolation of user input. Unit tests in [tests/onboarding-staging-crud.test.ts](/tests/onboarding-staging-crud.test.ts) and [tests/onboarding-apply.test.ts](/tests/onboarding-apply.test.ts); DB-backed integration tests in [tests/onboarding-staging-db-integration.test.ts](/tests/onboarding-staging-db-integration.test.ts) and [tests/onboarding-apply-integration.test.ts](/tests/onboarding-apply-integration.test.ts).
- **Apply**: `applyClientOnboardingStaging()` (in [lib/onboarding-staging-db.ts](/lib/onboarding-staging-db.ts)) is invoked by `processChangeForProcessedStatus` when the `customer_onboarding` change request is processed. It reads the staging row via `getClientOnboardingStagingByChangeRequestId()`, then in ONE transaction: skips when `client_config.client` already exists (idempotent), otherwise creates the parent account (if needed), the client, the portfolio, and the initial `portfolio_configuration` row (with `change_request_id` lineage), and flips the staging row to `applied` + `processed_at`. On any error the transaction rolls back (no partial live rows) and the staging row is set to `failed` with `apply_error`, so a re-process retries the apply.
